...
app.post('/mine/submit', rateLimit({ windowMs: 60000, max: 30 }), requireWeb3Auth, async (req, res) => {
    try {
        ...
        // Validate if the hash meets network difficulty
        if (!hash.startsWith('0'.repeat(nexusChain.difficulty))) {
            return res.status(400).json({ error: 'Invalid PoW hash rejected.' });
        }
...
app.post('/stake/claim', txLimiter, requireWeb3Auth, async (req, res) => {
     try {
        ...
        const totalReturn = parseFloat(stake.amount) + yieldAmount;

        const claimTx = {
            from: 'staking_pool', to: uid, amount: totalReturn,
            type: 'TRANSFER', tokenSymbol: 'SYR',
            timestamp: Date.now(), isSystemGenerated: true, description: `Stake Claim + Yield`
        };

        if (await mempool.addTransaction(claimTx)) {
            broadcastP2P({ type: 'BROADCAST_TX', data: claimTx });
            await pool.query('UPDATE staking_positions SET status = $1 WHERE id = $2', ['CLAIMED', stakeId]);
            res.json({ success: true, message: `Successfully claimed ${totalReturn.toFixed(4)} SYR.` });
        } else {
