...
app.get('/pricehistory', readLimiter, async (req, res) => {
    ...
    try {
        // FIX: was ORDER BY ASC which returned the OLDEST 500 trades.
        // For tokens with > 500 trades, the chart never showed recent price action.
        // Now returns the MOST RECENT 500 trades (DESC) then lets the client sort ASC.
        ...
        // NOTE: rows arrive DESC from DB; the client (fetchPriceHistory) always
        // re-sorts ASC by timestamp, so order here doesn't matter for the chart.
...
app.get('/api/chart/kline', async (req, res) => {
    try {
        ...
        // bucketed by the requested timeframe. This makes SYR candle/bar charts
        // work even though the DB rows are missing the amount_usd column value.
        if (formatted.length === 0 && symbol === 'SYR') {
            const cache = nexusChain.priceHistoryCache || [];
            if (cache.length > 0) {
                const bucketMs = (() => {
                    switch (timeframe) {
                        ...
                        case '45m': return 2_700_000;
                        case '2H':  return 7_200_000;
                        case '3H':  return 10_800_000;
                ...
                const filterFrom = sinceMs || 0;
                const buckets = new Map();
                cache.forEach(entry => {
                    const ts = entry.timestamp;
                    if (ts < filterFrom) return;
                    const bucket = Math.floor(ts / bucketMs) * bucketMs;
                    if (!buckets.has(bucket)) {
                        buckets.set(bucket, { time: bucket, open: entry.price, high: entry.price, low: entry.price, close: entry.price, volume: 1 });
                    } else {
                        const b = buckets.get(bucket);
                        b.high   = Math.max(b.high, entry.price);
                        b.low    = Math.min(b.low,  entry.price);
                        b.close  = entry.price;
                        b.volume += 1;
                    }
                });
                // If sinceMs filter wiped everything, retry with no filter (e.g. 1m on old chain)
                if (buckets.size === 0) {
                    cache.forEach(entry => {
                        const bucket = Math.floor(entry.timestamp / bucketMs) * bucketMs;
                        if (!buckets.has(bucket)) {
                            buckets.set(bucket, { time: bucket, open: entry.price, high: entry.price, low: entry.price, close: entry.price, volume: 1 });
                        } else {
                            const b = buckets.get(bucket);
                ...
                formatted = Array.from(buckets.values()).sort((a, b) => a.time - b.time);
            }
        }
...
app.post('/menubook/market', txLimiter, requireWeb3Auth, async (req, res) => {
    try {
        ...
        if (tokenSymbol !== 'SYR') {
            ...
            if (sandwich.blocked) {
                releaseTradeLock(uid);
                return res.status(429).json({ error: sandwich.reason });
            }
        }
        ...
        if (side === 'BUY' && matchResult.remaining > 1e-8 && tokenSymbol !== 'SYR') {
            try {
                ...
                if (handlerAddr) {
                    ...
                    // Initialize pool state (includes virtual reserve fields)
                    ...
                    if (lp.tokenReserve <= 0 || lp.usdReserve <= 0) {
                        ...
                        if (initialPrice > 0 && handlerBal > 0) {
                            ...
                            // Initialize virtual reserves if not already set
                            initVirtualReserves(lp, initialPrice);
                        }
                    }

                    // Use EFFECTIVE reserves (real + virtual) for AMM price
                    const effToken = lp.tokenReserve + (lp.virtualTokenReserve || 0);
                    const effUsd   = lp.usdReserve   + (lp.virtualUsdReserve || 0);

                    if (handlerBal > 1e-8 && effToken > 0 && effUsd > 0) {
                        ...
                        const ammStartPrice   = hardPeg || fixDust(effUsd / effToken);
                        let   chunkCount       = 0;
                        ...
                        while (totalRemaining > 1e-8 && totalHandlerBal > 1e-8 && budgetRemaining > 1e-8 && chunkCount < MAX_CHUNKS) {
                            chunkCount++;
                            const effNowT = lp.tokenReserve + (lp.virtualTokenReserve || 0);
                            const effNowU = lp.usdReserve   + (lp.virtualUsdReserve   || 0);
                            const currentPoolPrice = hardPeg || fixDust(effNowU / effNowT);
                            ...
                            const effAfterU = lp.usdReserve   + (lp.virtualUsdReserve   || 0);
                            const newChunkPrice = effAfterT > 0 ? fixDust(effAfterU / effAfterT) : currentPoolPrice;
                        ...
                        // Update menubook last trade price with final pool price (effective reserves)
                        const effFinalToken = lp.tokenReserve + (lp.virtualTokenReserve || 0);
                        const effFinalUsd   = lp.usdReserve   + (lp.virtualUsdReserve   || 0);
                        const finalPoolPrice = effFinalToken > 0 ? fixDust(effFinalUsd / effFinalToken) : ammStartPrice;
                        menuBook.books[tokenSymbol].lastTradePrice = finalPoolPrice;
                        apiCache.tokenStats.delete(tokenSymbol);

                        // AMM UPGRADE: Record TWAP observation and check graduation
                        updateTWAP(tokenSymbol, finalPoolPrice);
                        recordHumanTrade(tokenSymbol);
                        await checkGraduation(tokenSymbol, lp);
                ...
                if (handlerAddr) {
                    ...
                    if (lp.tokenReserve <= 0 || lp.usdReserve <= 0) {
                        ...
                        if (initPrice > 0 && handlerBal > 0) {
                            ...
                            initVirtualReserves(lp, initPrice);
                        }
                    }

                    // Use EFFECTIVE reserves for AMM pricing
                    const sEffT = lp.tokenReserve + (lp.virtualTokenReserve || 0);
                    const sEffU = lp.usdReserve   + (lp.virtualUsdReserve   || 0);

                    const handlerUsdBal = nexusChain.state.getUsd(handlerAddr) - mempool.getPendingUsdSpend(handlerAddr);

                    if (handlerUsdBal > 1e-8 && sEffT > 0 && sEffU > 0) {
                        // CHUNKED EXECUTION for sells
                        const chunkSize       = Math.max(100, fixDust(lp.tokenReserve * 0.001));
                        let   totalRemaining  = matchResult.remaining;
                        let   usdBudget       = handlerUsdBal;
                        let   slippageExceeded = false;
                        const ammStartPrice   = hardPeg || fixDust(sEffU / sEffT);
                        let   chunkCount       = 0;
                        const MAX_CHUNKS       = 500;

                        while (totalRemaining > 1e-8 && usdBudget > 1e-8 && chunkCount < MAX_CHUNKS) {
                            chunkCount++;
                            const sNowT = lp.tokenReserve + (lp.virtualTokenReserve || 0);
                            const sNowU = lp.usdReserve   + (lp.virtualUsdReserve   || 0);
                            const currentPoolPrice = hardPeg || fixDust(sNowU / sNowT);
                            ...
                            const sAfterU = lp.usdReserve   + (lp.virtualUsdReserve   || 0);
                            const newChunkPrice = sAfterT > 0 ? fixDust(sAfterU / sAfterT) : currentPoolPrice;
                        ...
                        const sFinalT = lp.tokenReserve + (lp.virtualTokenReserve || 0);
                        const sFinalU = lp.usdReserve   + (lp.virtualUsdReserve   || 0);
                        const finalPoolPrice = sFinalT > 0 ? fixDust(sFinalU / sFinalT) : ammStartPrice;
                        menuBook.books[tokenSymbol].lastTradePrice = finalPoolPrice;
                        apiCache.tokenStats.delete(tokenSymbol);

                        // AMM UPGRADE: Record TWAP and check graduation
                        updateTWAP(tokenSymbol, finalPoolPrice);
                        recordHumanTrade(tokenSymbol);
                        await checkGraduation(tokenSymbol, lp);
        ...
        // AMM UPGRADE: Release trade mutex lock
        releaseTradeLock(uid);
        ...
        releaseTradeLock(uid);  // Always release lock on error
        res.status(500).json({ error: 'Internal Server Error' });
    }
});
...
app.get('/stats', async (req, res) => {
    ...
    let tokenPrice = ['SDX', 'SDTX'].includes(token) ? 1.00 : (menuBook.books[token]?.lastTradePrice || nexusChain.state.getPoolPrice(token) || menuBook.books[token]?.asks?.[0]?.priceUsd || 0);
...
app.post('/stake/positions', readLimiter, requireWeb3Auth, async (req, res) => {
    try {
        const uid = req.user.uid;
        const stakes = await pool.query('SELECT * FROM staking_positions WHERE uid = $1 ORDER BY locked_at DESC', [uid]);
...
app.post('/mint-new-cash', txLimiter, requireWeb3Auth, async (req, res) => {
    ...
    try {
        ...
        await mempool.addTransaction({
            ...
            // POLICY: Launch price is always fixed at $0.00000001 regardless of
            // what the deploy form sends. This prevents market manipulation by
            // creators setting an artificially high initial price. The real price
            // is discovered naturally through order-book trading.
            priceUsd: 0,
