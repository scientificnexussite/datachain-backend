...
// ─── Stripe Identity Webhook (Must be before bodyParser) ──────────────────────
...
app.post('/api/crypto/generate-address', txLimiter, requireWeb3Auth, async (req, res) => {
    try {
        ...
        const payPayload = {
            price_amount: parseFloat(minAmount) || 5.00, // FIX 4: Real dynamic deposit amount instead of dummy 1M
            price_currency: 'usd',
...
app.post('/api/crypto/convert', txLimiter, requireWeb3Auth, async (req, res) => {
    try {
        ...
        if (await mempool.addTransaction(deductTx) && await mempool.addTransaction(creditTx)) {
...
// Route: Create Stripe Identity KYC Session
    ...
    try {
        ...
        await pool.query(
            `INSERT INTO merchant_kyc (uid, stripe_session_id, status, created_at) 
             ...
             ON CONFLICT (uid) DO UPDATE SET stripe_session_id = EXCLUDED.stripe_session_id, status = 'pending'`,
            [uid, verificationSession.id, Date.now()]
        );
