...
async function bootstrapSdtX() {
    ...
    if (!sdtxBal || Object.keys(sdtxBal).length === 0) {
        ...
        const seedTx = {
            from: 'system', to: 'system', amount: 1000000, amountUsd: 1000000,
            type: 'LIQUIDITY_INIT', tokenSymbol: 'SDTX',
...
// ─── Mobile Bearer Auth Middleware ───────────────────────────────────────────
// Used by the SilverWallet APK. Token = base64url(JSON{address,timestamp,sig})
// where sig = SHA-256(address:timestamp) signed with the secp256k1 private key.
const requireBearerAuth = async (req, res, next) => {
    const authHeader = req.headers['authorization'];
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return requireWeb3Auth(req, res, next); // fall back to body-based web auth
    }
    try {
        const tokenB64 = authHeader.slice(7);
        const payload = JSON.parse(Buffer.from(tokenB64, 'base64url').toString());
        const { address, timestamp, sig } = payload;
        if (!address || !timestamp || !sig)
            return res.status(401).json({ error: 'Invalid mobile token' });
        if (Math.abs(Date.now() - timestamp) > 10 * 60 * 1000)
            return res.status(401).json({ error: 'Mobile token expired' });
        if (address.length !== 66)
            return res.status(401).json({ error: 'Invalid address format' });
        // Build SPKI DER from compressed secp256k1 public key (address = pubkey hex)
        ...
        const verify = crypto.createVerify('SHA256');
        verify.update(`${address}:${timestamp}`);
        if (!verify.verify(publicKey, Buffer.from(sig, 'hex')))
            return res.status(401).json({ error: 'Invalid mobile token signature' });
        req.user = { uid: address };
        req.body = req.body || {};
        req.body.uid = address;
        next();
    } catch (e) {
        return res.status(401).json({ error: 'Mobile auth failed: ' + e.message });
    }
};
...
// GET /p2p/offers — list open offers; optional ?asset=SDX&payMethod=Bank
app.get('/p2p/offers', readLimiter, async (req, res) => {
    try {
        const { asset, payMethod, minAmount, maxAmount } = req.query;
        let q = 'SELECT * FROM p2p_offers WHERE status = $1';
        const params = ['OPEN'];
        if (asset) { q += ` AND asset_symbol = $${params.length + 1}`; params.push(asset.toUpperCase()); }
        if (payMethod) { q += ` AND pay_method ILIKE $${params.length + 1}`; params.push(payMethod); }
        if (minAmount) { q += ` AND amount >= $${params.length + 1}`; params.push(parseFloat(minAmount)); }
        if (maxAmount) { q += ` AND amount <= $${params.length + 1}`; params.push(parseFloat(maxAmount)); }
        q += ' ORDER BY created_at DESC LIMIT 100';
        const result = await pool.query(q, params);
        res.json(result.rows);
    } catch (e) { res.status(500).json({ error: 'Failed to fetch offers' }); }
});

// POST /p2p/post-offer — merchant posts an offer (locks their balance in the offer)
app.post('/p2p/post-offer', txLimiter, requireBearerAuth, async (req, res) => {
    try {
        const { merchantAddress, assetSymbol, amount, rate, currency, payMethod, payDetails } = req.body;
        const uid = req.user.uid;
        if (merchantAddress && merchantAddress !== uid)
            return res.status(403).json({ error: 'Forbidden: address mismatch' });
        const sym = String(assetSymbol || '').toUpperCase();
        if (!['SDX', 'SDTX'].includes(sym)) return res.status(400).json({ error: 'Only SDX and SDTX are supported for P2P' });
        const amt = parseFloat(amount);
        if (isNaN(amt) || amt <= 0) return res.status(400).json({ error: 'Invalid amount' });
        if (isNaN(parseFloat(rate)) || parseFloat(rate) <= 0) return res.status(400).json({ error: 'Invalid rate' });
        // Check balance
        const bal = sym === 'SDX' ? nexusChain.state.getUsd(uid) : nexusChain.state.getBalance(uid, sym);
        if (bal < amt) return res.status(400).json({ error: `Insufficient ${sym} balance` });
        await pool.query(
            `INSERT INTO p2p_offers (merchant_address, asset_symbol, amount, rate, currency, pay_method, pay_details, status, created_at)
             VALUES ($1,$2,$3,$4,$5,$6,$7,'OPEN',$8) RETURNING id`,
            [uid, sym, amt, parseFloat(rate), currency || 'USD', payMethod, payDetails || '', Date.now()]
        );
        res.status(201).json({ success: true, message: 'Offer posted successfully' });
    } catch (e) { res.status(500).json({ error: 'Failed to post offer: ' + e.message }); }
});

// POST /p2p/cancel-offer — merchant cancels their open offer
app.post('/p2p/cancel-offer', txLimiter, requireBearerAuth, async (req, res) => {
    try {
        const { offerId } = req.body;
        const uid = req.user.uid;
        const offerRes = await pool.query('SELECT * FROM p2p_offers WHERE id = $1', [parseInt(offerId)]);
        if (offerRes.rows.length === 0) return res.status(404).json({ error: 'Offer not found' });
        const offer = offerRes.rows[0];
        if (offer.merchant_address !== uid) return res.status(403).json({ error: 'Forbidden' });
        if (offer.status !== 'OPEN') return res.status(400).json({ error: 'Only OPEN offers can be cancelled' });
        await pool.query('UPDATE p2p_offers SET status = $1 WHERE id = $2', ['CANCELLED', offer.id]);
        res.json({ success: true });
    } catch (e) { res.status(500).json({ error: 'Failed to cancel offer' }); }
});

// POST /p2p/initiate-trade — buyer starts a trade, locks the offer
app.post('/p2p/initiate-trade', txLimiter, requireBearerAuth, async (req, res) => {
    try {
        const { offerId, buyerAddress } = req.body;
        const uid = req.user.uid;
        const buyer = buyerAddress || uid;
        if (buyer !== uid) return res.status(403).json({ error: 'Forbidden' });
        const offerRes = await pool.query('SELECT * FROM p2p_offers WHERE id = $1 FOR UPDATE', [parseInt(offerId)]);
        if (offerRes.rows.length === 0) return res.status(404).json({ error: 'Offer not found' });
        const offer = offerRes.rows[0];
        if (offer.status !== 'OPEN') return res.status(400).json({ error: 'Offer is no longer available' });
        if (offer.merchant_address === uid) return res.status(400).json({ error: 'Cannot trade your own offer' });
        await pool.query('UPDATE p2p_offers SET status = $1 WHERE id = $2', ['LOCKED', offer.id]);
        const tradeRes = await pool.query(
            ...
            [offer.id, uid, offer.merchant_address, offer.asset_symbol, offer.amount, offer.rate,
             offer.currency, offer.pay_method, offer.pay_details, Date.now()]
        );
        res.status(201).json({ tradeId: tradeRes.rows[0].id, offer });
    } catch (e) { res.status(500).json({ error: 'Failed to initiate trade: ' + e.message }); }
});

// GET /p2p/trade/:id — trade status (buyer and merchant can both view)
app.get('/p2p/trade/:id', readLimiter, async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM p2p_trades WHERE id = $1', [parseInt(req.params.id)]);
        if (result.rows.length === 0) return res.status(404).json({ error: 'Trade not found' });
        res.json(result.rows[0]);
    } catch (e) { res.status(500).json({ error: 'Failed to fetch trade' }); }
});

// POST /p2p/mark-paid — buyer marks fiat payment sent
app.post('/p2p/mark-paid', txLimiter, requireBearerAuth, async (req, res) => {
    try {
        const { tradeId } = req.body;
        const uid = req.user.uid;
        const result = await pool.query('SELECT * FROM p2p_trades WHERE id = $1', [parseInt(tradeId)]);
        if (result.rows.length === 0) return res.status(404).json({ error: 'Trade not found' });
        const trade = result.rows[0];
        if (trade.buyer_address !== uid) return res.status(403).json({ error: 'Only the buyer can mark payment' });
        if (trade.status !== 'PENDING') return res.status(400).json({ error: 'Trade is not in PENDING state' });
        await pool.query('UPDATE p2p_trades SET status = $1, updated_at = $2 WHERE id = $3',
            ['PAID', Date.now(), trade.id]);
        res.json({ success: true, status: 'PAID' });
    } catch (e) { res.status(500).json({ error: 'Failed to mark paid' }); }
});

// POST /p2p/release-funds — merchant confirms receipt and releases crypto to buyer
app.post('/p2p/release-funds', txLimiter, requireBearerAuth, async (req, res) => {
    try {
        const { tradeId } = req.body;
        const uid = req.user.uid;
        const result = await pool.query('SELECT * FROM p2p_trades WHERE id = $1', [parseInt(tradeId)]);
        if (result.rows.length === 0) return res.status(404).json({ error: 'Trade not found' });
        const trade = result.rows[0];
        if (trade.merchant_address !== uid) return res.status(403).json({ error: 'Only the merchant can release funds' });
        if (trade.status !== 'PAID') return res.status(400).json({ error: 'Trade must be in PAID state to release' });
        // Execute the on-chain transfer (SDX uses USD balance; SDTX uses token balance)
        if (trade.asset_symbol === 'SDX') {
            await mempool.addTransaction({
                from: trade.merchant_address, to: 'system', amount: trade.amount,
                type: 'USD_WITHDRAWAL', timestamp: Date.now(), isSystemGenerated: true,
                description: `P2P trade #${trade.id} escrow debit`
            });
            await mempool.addTransaction({
                from: 'paypal-gateway', to: trade.buyer_address, amount: trade.amount,
                type: 'USD_DEPOSIT', timestamp: Date.now() + 1, isSystemGenerated: true,
                description: `P2P trade #${trade.id} release`
            });
        } else {
            await mempool.addTransaction({
                from: trade.merchant_address, to: trade.buyer_address, amount: trade.amount,
                type: 'TRANSFER', tokenSymbol: trade.asset_symbol,
                timestamp: Date.now(), isSystemGenerated: true,
                description: `P2P trade #${trade.id}`
            });
        }
        await pool.query('UPDATE p2p_trades SET status = $1, updated_at = $2 WHERE id = $3',
            ['COMPLETED', Date.now(), trade.id]);
        await pool.query('UPDATE p2p_offers SET status = $1 WHERE id = $2', ['COMPLETED', trade.offer_id]);
        // Update merchant stats
        await pool.query('UPDATE p2p_offers SET trade_count = trade_count + 1 WHERE merchant_address = $1', [uid]);
        console.log(chalk.green(`[P2P] Trade #${trade.id} completed: ${trade.amount} ${trade.asset_symbol} → ${trade.buyer_address.substring(0,12)}...`));
        res.json({ success: true, status: 'COMPLETED' });
    } catch (e) { res.status(500).json({ error: 'Failed to release funds: ' + e.message }); }
});

// POST /p2p/dispute — raise a dispute on a trade
app.post('/p2p/dispute', txLimiter, requireBearerAuth, async (req, res) => {
    try {
        ...
        if (result.rows.length === 0) return res.status(404).json({ error: 'Trade not found' });
        const trade = result.rows[0];
        if (trade.buyer_address !== uid && trade.merchant_address !== uid)
            return res.status(403).json({ error: 'Not a participant in this trade' });
        if (['COMPLETED', 'DISPUTED'].includes(trade.status))
            return res.status(400).json({ error: 'Cannot dispute a ' + trade.status + ' trade' });
        await pool.query('UPDATE p2p_trades SET status = $1, dispute_reason = $2, updated_at = $3 WHERE id = $4',
            ['DISPUTED', reason || 'No reason provided', Date.now(), trade.id]);
        res.json({ success: true, status: 'DISPUTED' });
    } catch (e) { res.status(500).json({ error: 'Failed to raise dispute' }); }
});

// GET /p2p/my-trades — authenticated user's active + completed trades
app.get('/p2p/my-trades', requireBearerAuth, async (req, res) => {
    try {
        const uid = req.user.uid;
        const result = await pool.query(
            'SELECT * FROM p2p_trades WHERE buyer_address = $1 OR merchant_address = $1 ORDER BY created_at DESC LIMIT 50',
            [uid]
        );
        res.json(result.rows);
    } catch (e) { res.status(500).json({ error: 'Failed to fetch trades' }); }
});

// POST /p2p/rate-trade — submit a rating after trade completes
app.post('/p2p/rate-trade', txLimiter, requireBearerAuth, async (req, res) => {
    try {
        const { tradeId, rating } = req.body;
        const uid = req.user.uid;
        ...
        if (trade.buyer_address === uid) {
            await pool.query('UPDATE p2p_trades SET buyer_rating = $1 WHERE id = $2', [parseInt(rating), trade.id]);
        } else if (trade.merchant_address === uid) {
            await pool.query('UPDATE p2p_trades SET merchant_rating_val = $1 WHERE id = $2', [parseInt(rating), trade.id]);
            // Update merchant's average rating on their offers
            const avgRes = await pool.query(
                'SELECT AVG(buyer_rating) as avg FROM p2p_trades WHERE merchant_address = $1 AND buyer_rating IS NOT NULL',
                [uid]
            );
            const newAvg = parseFloat(avgRes.rows[0].avg) || 5.0;
            await pool.query('UPDATE p2p_offers SET merchant_rating = $1 WHERE merchant_address = $2', [newAvg, uid]);
        } else {
            return res.status(403).json({ error: 'Not a participant in this trade' });
        }
        res.json({ success: true });
    } catch (e) { res.status(500).json({ error: 'Failed to rate trade' }); }
});

// GET /p2p/merchant/:address — merchant profile (rating, trade count, active offers)
app.get('/p2p/merchant/:address', readLimiter, async (req, res) => {
    try {
        const address = req.params.address;
        const [offersRes, tradesRes] = await Promise.all([
            pool.query("SELECT * FROM p2p_offers WHERE merchant_address = $1 AND status = 'OPEN' LIMIT 20", [address]),
            pool.query('SELECT COUNT(*) as count, AVG(buyer_rating) as avg_rating FROM p2p_trades WHERE merchant_address = $1 AND status = $2', [address, 'COMPLETED'])
        ]);
        const stats = tradesRes.rows[0];
        res.json({
            address,
            tradeCount: parseInt(stats.count) || 0,
            rating: parseFloat(stats.avg_rating) || 5.0,
            activeOffers: offersRes.rows
        });
    } catch (e) { res.status(500).json({ error: 'Failed to fetch merchant profile' }); }
});
