...
app.post('/api/crypto/generate-address', txLimiter, requireWeb3Auth, async (req, res) => {
    try {
        ...
        const payPayload = {
            ...
            // for being "less than minimal". The webhook handles 'partially_paid' so users can deposit any amount.
...
app.post('/api/crypto/webhook', express.json(), async (req, res) => {
    try {
        ...
        // FIX: Accept 'partially_paid' because we requested a dummy $1000 to bypass minimums.
        // Users will almost always trigger 'partially_paid' by sending less than $1000.
        ...
        // FIX: Mint based on USD equivalent, not raw crypto token count (e.g. 20 MATIC != 20 SDTX)
        ...
        await pool.query(
            ...
            [payment_id.toString(), order_id, pay_currency, fiatAmount, 'COMPLETED', Date.now()]
        );

        // MINT the SDTX Stablecoin 1:1 with the deposit
        const mintTx = {
            from: 'system', to: order_id,
            amount: fiatAmount, type: 'MINT', tokenSymbol: 'SDTX',
            timestamp: Date.now(), isSystemGenerated: true, 
