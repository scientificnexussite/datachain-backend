...
app.get('/tokens', readLimiter, async (req, res) => {
    try {
        ...
        try {
            ...
            // Overlay dynamic ReamSett metadata (logo and description updates)
            const metaRes = await pool.query(
                `SELECT ticker, description, logo_base64 FROM token_metadata WHERE ticker = ANY($1)`,
                [tokensObj]
            );
...
// Helper to map UI networks to NowPayments USDT tickers
const getCryptoTicker = (network) => {
        const map = { 'TRX': 'usdttrc20', 'BSC': 'usdtbsc', 'ETH': 'usdterc20', 'APT': 'apt' };
    return map[network.toUpperCase()] || 'usdttrc20';
};

// Route 1: Generate Deposit Address & Fetch Minimums
app.post('/api/crypto/generate-address', txLimiter, requireWeb3Auth, async (req, res) => {
    try {
        ...
        // Fetch Minimum Deposit Amount
        let minAmount = 5.0;
        try {
            const minRes = await fetch(`${NOWPAYMENTS_API_BASE}/min-amount?currency_from=${currency}&currency_to=${currency}`, {
                headers: { 'x-api-key': NOWPAYMENTS_API_KEY }
            });
            const minData = await minRes.json();
            if (minData.min_amount) minAmount = minData.min_amount;
        } catch (e) {
        ...
        const payPayload = {
            ...
            order_id: uid,
            order_description: `SDTX Bridge Deposit for ${uid}`
        };
...
app.post('/api/crypto/webhook', express.json(), async (req, res) => {
    try {
        ...
        // We only process finished (confirmed) transactions
        if (payment_status !== 'finished') return res.status(200).send('Ignored: Not finished');
        ...
        await pool.query(
            ...
            [payment_id.toString(), order_id, pay_currency, actually_paid, 'COMPLETED', Date.now()]
        );

        // MINT the SDTX Stablecoin 1:1 with the deposit
        const mintTx = {
            from: 'system', to: order_id,
            amount: actually_paid, type: 'MINT', tokenSymbol: 'SDTX',
            timestamp: Date.now(), isSystemGenerated: true, 
...
// Route 3: Withdraw Crypto (Burns SDTX/SDX and requests Mass Payout)
app.post('/api/crypto/withdraw', txLimiter, requireWeb3Auth, async (req, res) => {
    try {
        ...
        // Check if user has sufficient stablecoins (Prefer SDTX, fallback to SDX)
        const sdtxBal = nexusChain.getBalance(uid, 'SDTX') - mempool.getPendingTokenSpend(uid, 'SDTX');
        const sdxBal  = nexusChain.getBalance(uid, 'SDX') - mempool.getPendingTokenSpend(uid, 'SDX');
        
        let burnSymbol = null;
        if (sdtxBal >= parsedAmount) burnSymbol = 'SDTX';
        else if (sdxBal >= parsedAmount) burnSymbol = 'SDX';

        if (!burnSymbol) return res.status(400).json({ error: 'Insufficient SDTX/SDX balance for this withdrawal.' });
        ...
        // BURN the internal stablecoin since the physical crypto is leaving the vault
        const burnTx = {
            from: uid, to: 'system',
            amount: parsedAmount, type: 'TRANSFER', tokenSymbol: burnSymbol,
            timestamp: Date.now(), isSystemGenerated: true, 
...
app.post('/api/token/update-settings', txLimiter, requireWeb3Auth, async (req, res) => {
    try {
        const { ticker, description, logoBase64, fdxTiers } = req.body;
        const uid = req.user.uid;
        ...
        await pool.query(
            `INSERT INTO token_metadata (ticker, description, logo_base64, fdx_tiers, updated_at) 
             VALUES ($1, $2, $3, $4, $5) 
            ...
            [ticker.toUpperCase(), description || null, logoBase64 || null, fdxTiers ? JSON.stringify(fdxTiers) : null, Date.now()]
        );
...
// ─── PHASE 8: Stablecoin Bridge (SDX <-> SDTX Converter) ──────────────────────
app.post('/api/crypto/convert', txLimiter, requireWeb3Auth, async (req, res) => {
    try {
        ...
        if (!validCoins.includes(fromUpper) || !validCoins.includes(toUpper) || fromUpper === toUpper) {
            return res.status(400).json({ error: 'Conversion is only supported between SDX and SDTX.' });
        }

        // Check user balance
        const availableBal = nexusChain.getBalance(uid, fromUpper) - mempool.getPendingTokenSpend(uid, fromUpper);
        ...
        // 1:1 Conversion: Burn the fromCoin, Mint the toCoin
        const burnTx = {
            from: uid, to: 'system', amount: parsedAmount, type: 'TRANSFER', tokenSymbol: fromUpper,
            timestamp: Date.now(), isSystemGenerated: true, description: `${fromUpper} to ${toUpper} Bridge Burn`
        };
        const mintTx = {
            from: 'system', to: uid, amount: parsedAmount, type: 'MINT', tokenSymbol: toUpper,
            timestamp: Date.now() + 1, isSystemGenerated: true, description: `${fromUpper} to ${toUpper} Bridge Mint`
        };

        if (await mempool.addTransaction(burnTx) && await mempool.addTransaction(mintTx)) {
            broadcastP2P({ type: 'BROADCAST_TX', data: burnTx });
            broadcastP2P({ type: 'BROADCAST_TX', data: mintTx });
            res.status(201).json({ success: true, message: `Successfully converted ${parsedAmount} ${fromUpper} to ${toUpper}.` });
        } else {
            res.status(400).json({ error: 'Conversion failed. Mempool might be full.' });
        }
    } catch (err) {
