...
const getCryptoTicker = (network) => {
    const map = { 'TRX': 'usdttrc20', 'BSC': 'usdtbsc', 'ETH': 'usdt', 'APT': 'apt' };
...
app.post('/api/crypto/generate-address', txLimiter, requireWeb3Auth, async (req, res) => {
    try {
        ...
        const minAmount = minData.min_amount || 5.0; // Default fallback
        ...
        const payRes = await fetch(`${NOWPAYMENTS_API_BASE}/payment`, {
            ...
            body: JSON.stringify({
                price_amount: 1000000, // Dummy max target, user can send any amount
                price_currency: 'usd',
        ...
        if (!payData.pay_address) throw new Error(payData.message || 'Gateway rejected address generation.');
...
app.post('/api/crypto/withdraw', txLimiter, requireWeb3Auth, async (req, res) => {
    try {
        ...
        if (payoutData.error) throw new Error(payoutData.message || 'Gateway rejected payout request.');
