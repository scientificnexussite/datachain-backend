...
class Block {
  constructor(index, timestamp, data, previousHash = '') {
    ...
    // LIMITATION 18 FIX: Implement Merkle Root for cryptographically secure, lightweight block verification
  ...
  calculateMerkleRoot(txs) {
    if (typeof txs === 'string') return CryptoJS.SHA256(txs).toString();
    if (!Array.isArray(txs) || txs.length === 0) return CryptoJS.SHA256("empty").toString();
    
    // Hash each transaction individually
    let hashes = txs.map(tx => CryptoJS.SHA256(JSON.stringify(tx)).toString());
    
    // Pairwise hash up the tree until only the root remains
    while (hashes.length > 1) {
        if (hashes.length % 2 !== 0) hashes.push(hashes[hashes.length - 1]); // Duplicate last hash if odd number
        const nextLevel = [];
  ...
  calculateHash() {
    // Hashes the Merkle Root instead of the massive raw JSON payload
    // Fallback to stringified data for historical blocks minted before the Merkle Hard Fork
    const payload = this.merkleRoot ? this.merkleRoot : JSON.stringify(this.data);
    return CryptoJS.SHA256(
      this.index + this.previousHash + this.timestamp + payload + this.nonce
    ).toString();
  }
