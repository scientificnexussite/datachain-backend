...
class MenuBook {
  ...
  async addLimitOrder(uid, side, amountSyr, priceUsd, token = 'SYR') {
    ...
    const order = {
      ...
      // FEATURE 16 FIX: Store the original amount to calculate partial fill percentages
      originalAmountSyr: fixDust(amountSyr),
      priceUsd: fixDust(priceUsd),
      timestamp: Date.now(),
      // FEATURE 19: 30-Day Limit Order Auto-Expiry
  ...
  // ── FEATURE 18: Bulk Cancel ────────────────────────────────────────────────
  async cancelAllOrders(uid, targetToken = null) {
      await this.ensureLoaded();
      let count = 0;
      
      const tokensToClear = targetToken ? [targetToken] : Object.keys(this.books);
      
      for (const token of tokensToClear) {
          this._initTokenBook(token);
          
          const initialBids = this.books[token].bids.length;
          this.books[token].bids = this.books[token].bids.filter(b => b.uid !== uid);
          count += (initialBids - this.books[token].bids.length);

          const initialAsks = this.books[token].asks.length;
          this.books[token].asks = this.books[token].asks.filter(a => a.uid !== uid);
          count += (initialAsks - this.books[token].asks.length);
          
          if (count > 0) await this.saveOrders(token);
      }
      return count;
  }

  // ── FEATURE 19: Expiry Cleanup ─────────────────────────────────────────────
  async cleanupExpiredOrders() {
      ...
      for (const token in this.books) {
          const book = this.books[token];
          
          const initialBids = book.bids.length;
          book.bids = book.bids.filter(o => !o.expiresAt || o.expiresAt > now);
          if (initialBids !== book.bids.length) hasChanges = true;

          const initialAsks = book.asks.length;
          book.asks = book.asks.filter(o => !o.expiresAt || o.expiresAt > now);
          if (initialAsks !== book.asks.length) hasChanges = true;
          
          if (hasChanges) await this.saveOrders(token);
      }
      return hasChanges;
  }
}
